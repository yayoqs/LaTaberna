...
function _asegurarVista() {
    ...
    <div class="view-toolbar">
      ...
      <div class="toolbar-actions">
        <div class="recetas-search">
          ...
          <input type="text" id="recetasSearch" placeholder="Buscar receta...">
        </div>
        <div class="recetas-search" style="margin-left:8px;">
          <i class="fas fa-boxes"></i>
          <input type="text" id="recetasIngredienteSearch" placeholder="Buscar por ingrediente...">
        </div>
        <button class="btn-secondary" id="btnNuevaReceta"><i class="fas fa-plus"></i> Nueva Receta</button>
      </div>
    </div>
  ...
  document.getElementById('recetasIngredienteSearch').addEventListener('input', () => {
...
export function render(modo) {
  ...
  document.querySelectorAll('.receta-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      if (prodId) mostrarDetalle(prodId);
    });
  });
}
...
export function mostrarDetalle(prodId) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal-receta-detalle">
        <div class="modal-header">
          ...
          <button class="modal-close" id="btnCerrarDetalle"><i class="fas fa-times"></i></button>
        </div>
        ...
        <div class="modal-footer" id="detalleFooter">
          ...
          <button class="btn-primary" id="btnEditarReceta"><i class="fas fa-edit"></i> Editar Receta</button>
        </div>
      </div>
    `;
    ...
    document.getElementById('btnExportarPDF').addEventListener('click', () => exportarRecetaPDF(prodId));
    document.getElementById('btnEditarReceta').addEventListener('click', () => editarRecetaDesdeDetalle(prodId));
  }
...
function _asegurarModalReceta() {
  ...
  modal.innerHTML = `
    <div class="modal-small" style="max-width: 520px;">
      <div class="modal-header"><h3>Asignar Receta</h3><button class="modal-close" id="btnCerrarModalReceta"><i class="fas fa-times"></i></button></div>
      <div class="modal-small-body">
        ...
        <div class="modal-small-footer">
          <button class="btn-secondary" id="btnCancelarReceta">Cancelar</button>
          <button class="btn-primary" id="btnGuardarReceta">Guardar</button>
        </div>
      </div>
    </div>`;
  document.body.appendChild(modal);

  document.getElementById('btnCerrarModalReceta').addEventListener('click', cerrarModalReceta);
  document.getElementById('btnCancelarReceta').addEventListener('click', cerrarModalReceta);
  document.getElementById('btnGuardarReceta').addEventListener('click', guardarReceta);
  document.getElementById('recProductoId').addEventListener('change', _mostrarRecetaActual);
}
...
export function _mostrarRecetaActual() {
  ...
  if (receta && receta.ingredientes.length) {
    ...
    receta.ingredientes.forEach(ing => {
      ...
      html += `<li>${nombreIng}: ${ing.cantidad} <button class="btn-icon-sm" data-quitar-ing="${ing.ingredienteId}"><i class="fas fa-trash"></i></button></li>`;
    });
    html += '</ul>';
    div.innerHTML = html;

    div.querySelectorAll('[data-quitar-ing]').forEach(btn => {
      btn.addEventListener('click', () => {
        const ingId = btn.dataset.quitarIng;
        _quitarIngrediente(productoId, ingId);
      });
    });
  } else {
...
export async function _quitarIngrediente(productoId, ingredienteId) {
  if (_modo !== 'administracion') {
    showToast('error', 'Acceso denegado');
    return;
  }
