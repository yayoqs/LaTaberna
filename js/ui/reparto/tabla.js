...
export function asegurarVista(onNuevoPedido, onEditarItems, onEnviarCocina, onDespachar, onConfirmarEntrega, onEliminarPedido) {
  ...
  main.innerHTML = `
    <div class="view-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <button class="btn-primary" id="btnNuevoPedidoReparto"><i class="fas fa-plus"></i> Nuevo Pedido</button>
      </div>
    </div>
  ...
  if (btnNuevo) btnNuevo.addEventListener('click', onNuevoPedido);
  ...
  if (tbody) {
    tbody.addEventListener('click', function (e) {
      ...
      if (btn.classList.contains('btn-editar-items') && typeof onEditarItems === 'function') onEditarItems(deliveryId);
      else if (btn.classList.contains('btn-enviar-cocina') && typeof onEnviarCocina === 'function') onEnviarCocina(deliveryId);
      else if (btn.classList.contains('btn-despachar') && typeof onDespachar === 'function') onDespachar(deliveryId);
      else if (btn.classList.contains('btn-confirmar-entrega') && typeof onConfirmarEntrega === 'function') onConfirmarEntrega(deliveryId);
      else if (btn.classList.contains('btn-eliminar-delivery') && typeof onEliminarPedido === 'function') onEliminarPedido(deliveryId);
    });
  }
}
...
export function renderTabla(pedidos) {
  const tbody = document.getElementById('repartoBody');
  if (!tbody) return;
  if (_estadoFiltro !== 'todos') pedidos = pedidos.filter(p => p.estado === _estadoFiltro);
  if (!pedidos.length) {
  ...
  tbody.innerHTML = ordenados.map(p => {
    ...
    let btnEditar = '';
    if (p.estado === 'pendiente') btnEditar = `<button class="btn-ajuste btn-editar-items" data-id="${idSeguro}"><i class="fas fa-edit"></i> Editar</button>`;
    let botones = '';
    if (p.estado === 'pendiente') botones += `<button class="btn-ajuste btn-enviar-cocina" data-id="${idSeguro}"><i class="fas fa-fire-burner"></i> Enviar a Cocina</button>`;
    else if (p.estado === 'en_preparacion') botones += `<button class="btn-ajuste btn-despachar" data-id="${idSeguro}"><i class="fas fa-motorcycle"></i> En camino</button>`;
    else if (p.estado === 'en_camino') botones += `<button class="btn-ajuste btn-confirmar-entrega" data-id="${idSeguro}"><i class="fas fa-check"></i> Entregado</button>`;
