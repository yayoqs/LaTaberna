...
/**
 * Observable class - Patrón Observer para eventos
 * 
 * Proporciona un sistema de suscripción a eventos type-safe
 * Métodos principales:
 * - on(eventName, handler): Suscribirse a un evento
 * - once(eventName, handler): Suscribirse a un evento una sola vez
 * - off(eventName, handler): Desuscribirse de un evento
 * - fire(eventName, options): Disparar un evento
 * 
 * @example
 * class MyCanvas extends Observable<CanvasEvents> {
 *   fireMouseDown() {
...
export class Observable<EventSpec> {
   ...
   * @return {Function} disposer - Función para desuscribirse
   */
   ...
   * @param {EventRegistryObject} handlers key/value pairs (eg. {'after:render': handler, 'selection:cleared': handler})
   * @param {Function} handler Function that receives a notification when an event of the specified type occurs
   * @return {Function} disposer - Función para cancelar la suscripción
   */
